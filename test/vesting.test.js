...
describe("TokenVesting", function () {
  ...
  describe("Vesting", function () {
    ...
    it("Should vest tokens gradually", async function () {
      ...
      await expect(
        ...
        .to.emit(testToken, "Transfer")
        .withArgs(await tokenVesting.getAddress(), beneficiary.address, 10);
      ...
      await expect(
        ...
        .to.emit(testToken, "Transfer")
        .withArgs(await tokenVesting.getAddress(), beneficiary.address, 45);
    ...
    it("Should not release tokens before cliff", async function () {
      ...
      expect(
        await tokenVesting.computeReleasableAmount(vestingScheduleId)
      ).to.be.equal(30); // as time elapsed is 200 seconds + 100 seconds of cliff. So for cliff period 10 tokens gets unlocked
    });
    ...
    //   ).to.be.equal(0);
      ...
      const halfwayThrough = baseTime + duration / 2;
  ...
  //  For first 3 months, vested / claimable amount should be 0. On the first day of 4 th month,
  //  claimable amount should be 300.

  it("Should vest tokens correctly with a 3-months cliff and 12-months duration", async function () {
    // deploy vesting contract
    const tokenVesting = await TokenVesting.deploy(await testToken.getAddress());
    await tokenVesting.waitForDeployment();
    ...
    const baseTime = 0; // June 1, 2021
    const beneficiary = addr1;
    const startTime = baseTime;
    const cliff = 3*30*86400; // 3 months in seconds
    const duration = 10*30*86400; // 12 months in seconds

    // Assuming the tokens are vested monthly
    const slicePeriodSeconds = 30*86400; // 1 month in seconds
    const revokable = true;
    const amount = 1000;
    ...
    // set time to the cliff (3 months)
    await tokenVesting.setCurrentTime(baseTime + cliff + 1);
    ...
    // set time to halfway through the vesting period (10 months)
    const halfwayThrough = baseTime + duration / 2;
    await tokenVesting.setCurrentTime(halfwayThrough);
